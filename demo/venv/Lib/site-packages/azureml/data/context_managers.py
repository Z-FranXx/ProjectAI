...
class DatasetContextManager(_CommonContextManager):
        ...
        if dataset:
            ...
            if not isinstance(dataset, FileDataset):
                ...
                if self._is_download(data_configuration):
                    ...
                    is_single = self._mount_readonly(name, dataset, target_path)
                    action = "Mounting"
                if force_folder != "True" and is_single:
                    self.__class__._update_env_var_for_single_file_dataset(
                        data_configuration.environment_variable_name,
                        dataset,
                        env_name_prefix=self._input_env_var_prefix)
                else:
    ...
    def _mount_readonly(self, name, dataset, target_path):
        ...
        try:
            ...
            return self.__class__._is_single_file_context(context_manager)
    ...
    @staticmethod
    def _is_single_file_context(context):
        return True if getattr(context, 'is_single_file', getattr(context, 'single_file', None)) else False
    ...
    def _register_output(self, run_name, dest, config):
        ...
        if dataflow:
            ...
            from azureml.data._dataprep_helper import dataprep_stub, _convert_dataflow_stub_to_engineless

            transformations = dataprep_stub().DataflowStub.from_json(dataflow)
            engineless_transformations = _convert_dataflow_stub_to_engineless(transformations)
            combined = dataset._dataflow + engineless_transformations
            dataset = None
            if dataprep_stub().is_tabular(transformations):
                _log_and_print("Outputting a tabular dataset")
                dataset = TabularDataset._create(combined)
            else:
                _log_and_print("Outputting a file dataset")
                dataset = FileDataset._create(combined)

            has_partition_format = False
            for step in transformations._get_steps():
                has_partition_format |= step['type'] == 'Microsoft.DPrep.AddColumnsFromPartitionFormatBlock'
    ...
    @staticmethod
    def _update_env_var_for_single_file_dataset(env_name, dataset, env_name_prefix=''):
